...
btnPesan.addEventListener("click", kirimPesan);
...
var swalo = Swal.mixin({ allowOutsideClick: false, confirmButtonText: "Next", imageHeight: 150 });
async function fotoPopUp() {
  kirimPesan();
}

async function kirimPesan() {
  var { value: mypesan } = await swalo.fire({ title: "Tulis pesan disini ya ^_^", input: "textarea", confirmButtonText: "Kirim" });
  if (mypesan) {
    swalo.fire({
    ...
    pesanku = "Status:<br>" + pesan + "<br><br>Pesan:<br>" + mypesan;

    var data = new FormData();
    data.append("p", pesanku);
    ...
    await swalo.fire({ confirmButtonText: "Iya deh", title: "Jangan dikosongin ya :)" });
    kirimPesan();
  }
}
